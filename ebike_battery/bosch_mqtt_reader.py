  ...
  if(L.cloud||AL.length){$('#actCard').style.display='';
    if(AL.length){
      ...
      $('#actList').innerHTML=AL.slice().reverse().slice(0,25).map(e=>{
        ...
        return `<div style="padding:7px 0;border-top:1px solid var(--line);font-size:14px"><span class=muted>${ago(e.ts)}</span> — ${ico[e.type]||'•'} ${lab[e.type]||e.type}${dist}${loc}</div>`;}).join('');
